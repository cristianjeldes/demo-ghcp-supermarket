<#
Agent wrapper script: agent.ps1
Usage examples:
  .\agent.ps1 -Items "pan integral","leche" -Sites "super","jumbo"
  .\agent.ps1 -Prompt "Find prices for pan integral and leche" -Sites "super"

This script parses a freeform prompt into item list (simple split on commas / 'and') and delegates to scrape-prices.ps1.
#>
...
Write-Host "Agent: scraping items: $($Items -join ', ') on sites: $($Sites -join ', ')"
...
# Save parsed requests for downstream matching/inspection
$requestsPath = Join-Path $scriptDir ('requests-' + (Get-Date -UFormat %s) + '.json')
...
# Determine results CSV path
$resultsCsv = if ($Output) { $Output } else { Join-Path $scriptDir 'results.csv' }
# Run scraper
