// scraper.js
// Usage: node scraper.js <site> <query> [--headed]
// Sites supported: 'super' (super.lider.cl) and 'jumbo' (www.jumbo.cl)

const { chromium } = require('playwright');

function usage() {
  console.error('Usage: node scraper.js <site> <query> [--headed]');
  process.exit(2);
}
...
(async () => {
  try {
    ...
    // parse optional timeout from --timeout=MS or --timeout MS (defaults to 60000)
    let timeoutMs = 60000;
    ...
    // remove known boolean flags so they don't contaminate the query
    ['--headed','--no-headless','--head'].forEach(f => {
      let idx;
      while ((idx = argv.indexOf(f)) !== -1) argv.splice(idx,1);
    });
    ...
    // Force headed for 'lider' (super.lider.cl) because site serves different content to headless browsers
    if (siteKey === 'lider') {
      headless = false;
    }
    ...
    try {
      await page.waitForSelector(selector, { timeout: Math.max(15000, Math.floor(timeoutMs/2)) });
    } catch (e) {
      // selector may not appear quickly; continue and attempt extraction anyway
    }
    ...
    if (!headless) {
      try {
        ...
        await page.screenshot({ path: `${fnameBase}.png`, fullPage: true });
        const html = await page.content();
        const fs = require('fs');
        fs.writeFileSync(`${fnameBase}.html`, html, 'utf8');
        console.error(JSON.stringify({ debug: 'saved_screenshot', screenshot: `${fnameBase}.png`, html: `${fnameBase}.html` }));
      } catch (e) {
    ...
    // marker-wrapped output so callers can reliably extract JSON even if the script logs other text
