<#
PowerShell wrapper: scrape-prices.ps1 (src)
Usage example:
  powershell -File .\src\scrape-prices.ps1 -Items "pan integral","leche" -Sites "super","jumbo" -Output artifacts\results.csv
#>
...
# Always save results into artifacts (unless explicit Output provided)
$finalOutput = if ($Output) { $Output } else { Join-Path $artifacts 'results.csv' }
$results | Export-Csv -Path $finalOutput -NoTypeInformation -Encoding UTF8
Write-Host "Saved results to $finalOutput"
